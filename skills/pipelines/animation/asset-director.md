...
| Meta skill | `skills/meta/asset-intake-protocol.md` | User asset checks, cost disclosure, generation approval |

## Process

> **Before generating any asset (narration, music, SFX, or video clips), read and follow `skills/meta/asset-intake-protocol.md` Part 2.** This protocol ensures the agent checks user assets first, presents cost disclosure, and gets approval before any generation.
...
### 1d. Video Clip Generation (Approach B / clip_video)

When `animation_mode == "clip_video"` or using `video_selector` for generated shots:

> **Keyframe-to-video routing:** When the approved `scene_plan.json` includes `start_frame` and/or `end_frame` for a scene, pass `start_frame.path` as `reference_image_path` and `end_frame.path` as `last_image_path` to `video_selector`. This anchors the AI video to the approved storyboard keyframes.
