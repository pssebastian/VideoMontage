...
6. **The assets gate reviews the storyboard — before any draft render.**
   ...
   **Visual storyboard keyframes at scene_plan gate.** When the `scene_plan`
   stage includes `start_frame` and `end_frame` keyframes (generated via the
   asset intake protocol in `skills/meta/asset-intake-protocol.md`), the
   approval presentation should display the visual storyboard filmstrip — start
   frame → camera direction arrow → end frame — per scene, not just the
   text-based scene plan. The Backlot board renders keyframe thumbnails
   automatically when `start_frame.path` / `end_frame.path` exist in the
   scene plan artifact.
