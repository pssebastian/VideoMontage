...
### Pre-Generation Service & Cost Disclosure Card (Binding)

Before ANY generation call — concept art, keyframes, narration, music, SFX, video clips — the agent must present a **Service & Cost Disclosure Card** and wait for explicit user approval. This applies to all pipelines, not just keyframe-enabled ones.

**Mandatory card fields:**

| Field | Description |
|-------|-------------|
| Asset Category | What is being generated (e.g., "Keyframe images", "Narration", "Background music") |
| Service / Provider | Tool name + resolved provider (e.g., "image_selector → openrouter_image") |
| Model Variant | Specific model (e.g., "FLUX.1 Pro", "GPT Image 2", "ElevenLabs Turbo v2.5") |
| Quantity & Units | How many items (e.g., "6 images", "3 voice segments", "1 music track") |
| Est. Unit Cost | Per-item cost if known |
| Est. Total Cost | Total estimated cost |
| Budget Balance | Remaining budget after this generation |
| Free/Cheaper Alternative | At least one alternative when available |

**Rules:**
- Batch related calls into a single card (e.g., all keyframes for scenes 1–6 as one approval).
- Log every approval as a `decision_log` entry with `category: "service_and_cost_approval"`.
- If budget would be exceeded, warn explicitly and do not proceed without user acknowledgment.
- If cost is unknown (provider doesn't report pricing), state so and request explicit go-ahead.

The full protocol is documented in `skills/meta/asset-intake-protocol.md`.
...
### Two-Part Asset Intake Protocol (Mandatory Gate)

The complete protocol lives in `skills/meta/asset-intake-protocol.md`. This section summarizes the two-part flow and when each part triggers.

**Part 1 — Visual Asset Intake & Keyframe Storyboard** (between `script` approval and `scene_plan` stage):

1. Walk the approved script and enumerate every visual entity (characters, locations, props, costumes).
2. Cross-reference against user-provided references in `projects/<id>/assets/references/`.
3. Present an **Asset Gap Report** — matched vs. missing entities, with options per item: provide, generate, or rewrite.
4. For generation: present a **Service & Cost Disclosure Card** and wait for approval.
5. Generate concept anchors for missing entities. Present the full concept kit for approval.
6. Generate start/end frame keyframes for each visual scene, conditioned on approved references.
7. Present the visual storyboard filmstrip for human approval. Support selective re-rolls.

**Part 2 — Non-Visual & Audio Asset Intake** (at the `assets` stage, after visual storyboard lock):

1. Enumerate all audio needs (voiceover, music, SFX, ambient) from the approved scene plan.
2. **Always check user assets first** — ask if the user has voice recordings, music tracks, or SFX packs.
3. Present an **Audio Gap Report** for missing audio, with the same provide/generate/skip options.
4. For generation: present a **Service & Cost Disclosure Card** and wait for approval.
5. Generate short samples (1-sentence voice, 15s music preview) for approval before batch synthesis.
6. Reconcile pronunciation, audio timing vs. visual timing drift, and multi-speaker voice allocation.

**Platform & Aspect Ratio Derivation** (carried over from the original contract):

The agent automatically derives the optimal aspect ratio from the target platform specified in the brief:
- **TikTok / YouTube Shorts / Instagram Reels / Stories**: `9:16 Vertical` (`1080x1920`) with safe-zone margins.
- **YouTube (Long-form) / Website / Desktop / TV / Vimeo**: `16:9 Widescreen` (`1920x1080` or `3840x2160`).
- **Instagram Feed / Facebook Feed / LinkedIn**: `4:5 Portrait` (`1080x1350`) or `1:1 Square` (`1080x1080`).

Present the derived aspect ratio and confirm with the user alongside the AI model selection.

**Reference Media Binding**: In `scene_plan.json`, bind each scene's `start_frame`, `end_frame`, and `source_reference_ids` to approved reference files so downstream tools receive exact inputs.
