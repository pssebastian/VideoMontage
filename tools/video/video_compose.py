...
class VideoCompose(BaseTool):
    ...
    def _compose(self, inputs: dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            if subtitle_path and Path(subtitle_path).exists():
                ...
                sub_escaped = sub_p.as_posix().replace(":", "\\:")
                if sub_p.suffix.lower() == ".ass" and not inputs.get("force_subtitle_style"):
                    vfilters.append(f"subtitles='{sub_escaped}'")
                else:
                    style = inputs.get("subtitle_style")
                    style_dict = style if isinstance(style, dict) else {}
                    ass_style = self._build_subtitle_style(style_dict)
    ...
    def _burn_subtitles(self, inputs: dict[str, Any]) -> ToolResult:
        ...
        sub_p = subtitle_path.resolve()
        sub_escaped = sub_p.as_posix().replace(":", "\\:")
        if sub_p.suffix.lower() == ".ass" and not inputs.get("force_subtitle_style"):
            vf_sub = f"subtitles='{sub_escaped}'"
        else:
            style = inputs.get("subtitle_style")
            style_dict = style if isinstance(style, dict) else {}
            ass_style = self._build_subtitle_style(style_dict)
            vf_sub = f"subtitles='{sub_escaped}':force_style='{ass_style}'"
        codec = inputs.get("codec", "libx264")
        crf = inputs.get("crf", 23)
