...
def _keyframe_visual(
    project_dir: Path,
    start_frame: Optional[dict],
) -> Optional[dict]:
    """Return a visual entry from a scene plan keyframe, or None."""
    if not start_frame or not isinstance(start_frame, dict):
        return None
    kf_path = start_frame.get("path")
    if not kf_path:
        return None
    resolved = _resolve_asset_path(project_dir, kf_path)
    if resolved is None:
        return None
    return {
        "type": "image",
        ...
        "renderable": resolved.exists() and resolved.suffix.lower() in {".png", ".jpg", ".jpeg", ".webp", ".gif"},
...
def _build_storyboard(
    ...
    for scene in scene_plan["scenes"]:
        ...
        # Keyframe fallback: if no manifest asset or snapshot exists, use
        # the scene plan's start_frame keyframe image (produced at scene_plan
        # stage before assets are generated).
        keyframe_visual = _keyframe_visual(project_dir, scene.get("start_frame"))
        active_visual = (
        ...
        cards.append({
            ...
            "keyframe_strategy": scene.get("keyframe_strategy"),
        })
